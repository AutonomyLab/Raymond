...
def MedianFilter(new_data):
	...
	if size % 2 == 1:
		median = sorted_dist[(size - 1) / 2]
	else:
		median = sum(sorted_dist[size/2 - 1:size/2 + 1])/2
	return median
...
while True:
	...
	if message_receive is not None:
		if len(message_receive) ==32:
			...
			distance = MedianFilter(raw_distance)
	...
	pose_x_odom, pose_y_odom, heading_odom = Localization(speed_odom, rot_speed_odom, pose_x_odom, pose_y_odom, heading_odom, direction_odom, loop_time)
  
  #compute next direction
	speed, rot_speed, direction = Algorithm(pose_x, pose_y, heading, distance)
	#speed, direction = DistanceControl(distance, pi/4, 0.4)
	#rot_speed = HeadingControl(heading)
