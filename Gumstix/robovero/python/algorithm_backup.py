	...
	if follow_init==True:
		...
		if diff_dist > threshold_outside_corner: #Outside corner
			...
			if diff_dist < threshold_inside_corner: # in [m]: #Inside corner
				...
				if start_curve > 150: # //last step was already the start of the curve to avoid problem after inside corner
					rot_speed = 0.8
					speed = distance*1/4.0
			...
			if follow_step == 0:
				follow_step = 0 
				start_curve = 0
				perpendicular = None #WallEstimation(distance, heading, pose_x, pose_y)# estimate the position of the wall
		...
		print '\t\tFollow wall: direction %d \t Rotspeed %0.2f\t Heading %d\t Distance %0.2f' % (degrees(direction), rot_speed, degrees(heading),distance)
	else:
...
def WallEstimation(distance, heading, pose_x, pose_y):
	...
	if estimation_index>estimation_data_length-1:
		...
		store_estimation = open("wall_estimation.txt","w")
		for n in range(estimation_data_length):
			store_estimation.write("%s \t%s \t %s \t %s \n" % (estimation_wall_x[n], estimation_wall_y[n], estimation_pose_x[n], estimation_pose_y[n]) )
