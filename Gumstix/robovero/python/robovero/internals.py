...
myTimeOut = 10 # 10 seconds
store_data = open("data.txt","w")
...
def robocaller(function, ret_type, *args):
  ...
  with robovero.lock:
    ...
    robovero.serial.flushInput() #Empty the input buffer before the next answer
...
class Robovero(object):
  ...
  def readline(self, delim):
    ...
    if myTimeOut == 10:
      if time_start-self.start_time > 7: # for the first 1.5 seconds the timeout is long but after it is short
        myTimeOut = 0.1
